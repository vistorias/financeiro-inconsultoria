...
with cA:
    st_kpi(
        "Entradas",
        fmt_brl(fluxo_disp["ENTRADAS"].sum()),
...
# Saldo acumulado com filtro de banco
with cD:
...
with cE:
    ...
    # histórico completo para cálculo real
    ent_hist = df_ent.copy()
    sai_hist = df_sai.copy()
    trf_hist = df_trf.copy()

    # aplica filtro de captação nas entradas
    if capt_sel and ("CAPTACAO" in ent_hist.columns):
        ent_hist = ent_hist[ent_hist["CAPTACAO"].isin([_upper(x) for x in capt_sel])].copy()
    ...
    # fluxo conforme filtro de bancos do painel
    fluxo_disp = build_fluxo_total_from_mv(
        mv_banks_daily,
        banco_sel,
        dt_ini,
        dt_fim
    ...
    if fluxo_disp.empty:
        ...
        fluxo_todos = build_fluxo_total_from_mv(
            mv_banks_daily,
            [],
            dt_ini,
            dt_fim
        ...
        saldo_todos = 0.0
        if not fluxo_todos.empty:
            saldo_todos = float(fluxo_todos.sort_values("DATA")["SALDO_REAL"].iloc[-1])
        ...
        with cD:
            ...
            badge = ("positivo", "good") if saldo_filtro >= 0 else ("negativo", "bad")
            st_kpi("Saldo acumulado (filtro)", fmt_brl(saldo_filtro), sub="Bancos selecionados", badge=badge)
        ...
        if resumo_banks is None or resumo_banks.empty:
            ...
            if banco_sel:
                show = show[show["BANCO"].isin([_upper(x) for x in banco_sel])].copy()
