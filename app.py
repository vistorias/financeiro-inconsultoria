...
def read_tab(sheet_id: str, tab: str) -> pd.DataFrame:
    """Leitura robusta.
    - evita erro com cabeçalhos vazios/duplicados
    - tolera linhas com quantidades diferentes de colunas
    - ajuda quando a base do Google Sheets está com filtro ativo
    """
    ...
    try:
        ...
        if use_conc:
            ...
            if dt_ini and dt_fim and (not conc_tbl.empty):
                conc_tbl = conc_tbl[
                    (conc_tbl["DATA"] >= dt_ini) & (conc_tbl["DATA"] <= dt_fim)
                ].copy()
    except Exception:
        conc_tbl = None
...
if conc_tbl is not None and (not conc_tbl.empty):
    ...
    if banco_sel:
        ...
        if not trf_hist.empty:
            if "ORIGEM" in trf_hist.columns and "DESTINO" in trf_hist.columns:
                trf_hist = trf_hist[
                    (trf_hist["ORIGEM"].isin(bset)) | (trf_hist["DESTINO"].isin(bset))
                ].copy()
    ...
    if fluxo_disp.empty:
        ...
        if not resumo_banks_show.empty and banco_sel:
            resumo_banks_show = resumo_banks_show[
                resumo_banks_show["BANCO"].isin([_upper(x) for x in banco_sel])
            ].copy()
